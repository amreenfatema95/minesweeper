...
public class MinesweeperGUI extends JFrame
{
	...
	//Details the set of actions to be executed when a button is double clicked.
	public void doubleClickAction(int x, int y)
	{
		position = newBoard.board[x][y].ordinal();
		
		if(countFlagsAround(x,y)==position)
		{
			if((x!=buttons.length-1)&&(buttons[x+1][y].getText()!="F")&&(buttons[x+1][y].isEnabled()==true))
			{
				buttons[x+1][y].doClick();
				leftClickAction(x+1,y);
			}
			if((x!=0)&&(buttons[x-1][y].getText()!="F")&&(buttons[x-1][y].isEnabled()==true))
			{
				buttons[x-1][y].doClick();
				leftClickAction(x-1,y);
			}
			if((y!=buttons[0].length-1)&&(buttons[x][y+1].getText()!="F")&&(buttons[x][y+1].isEnabled()==true))
			{
				buttons[x][y+1].doClick();
				leftClickAction(x,y+1);
			}
			if((y!=0)&&(buttons[x][y-1].getText()!="F")&&(buttons[x][y-1].isEnabled()==true))
			{
				buttons[x][y-1].doClick();
				leftClickAction(x,y-1);
			}
			if((x!=buttons.length-1)&&(y!=buttons[0].length-1)&&(buttons[x+1][y+1].getText()!="F")&&(buttons[x+1][y+1].isEnabled()==true))
			{
				buttons[x+1][y+1].doClick();
				leftClickAction(x+1,y+1);
			}
			if((x!=0)&&(y!=0)&&(buttons[x-1][y-1].getText()!="F")&&(buttons[x-1][y-1].isEnabled()==true))
			{
				buttons[x-1][y-1].doClick();
				leftClickAction(x-1,y-1);
			}
			if((x!=buttons.length-1)&&(y!=0)&&(buttons[x+1][y-1].getText()!="F")&&(buttons[x+1][y-1].isEnabled()==true))
			{
				buttons[x+1][y-1].doClick();
				leftClickAction(x+1,y-1);
			}
			if((x!=0)&&(y!=buttons[0].length-1)&&(buttons[x-1][y+1].getText()!="F")&&(buttons[x-1][y+1].isEnabled()==true))
			{
				buttons[x-1][y+1].doClick();
				leftClickAction(x-1,y+1);
			}
	...
	//Executes the left click actions for all buttons labelled zero surrounding a given button.
	public void checkZeroesAround(int i, int j)
	{
		if((i!=buttons.length-1)&&(buttons[i+1][j].getText()=="0")&&(buttons[i+1][j].isEnabled() == true))
		{
			leftClickAction(i+1,j);
		}
		if((i!=0)&&(buttons[i-1][j].getText()=="0")&&(buttons[i-1][j].isEnabled() == true))
		{
			leftClickAction(i-1,j);
		}
		if((j!=buttons[0].length-1)&&(buttons[i][j+1].getText()=="0")&&(buttons[i][j+1].isEnabled() == true))
		{
			leftClickAction(i,j+1);
		}
		if((j!=0)&&(buttons[i][j-1].getText()=="0")&&(buttons[i][j-1].isEnabled() == true))
		{
			leftClickAction(i,j-1);
		}
		if((i!=buttons.length-1)&&(j!=buttons[0].length-1)&&(buttons[i+1][j+1].getText()=="0")&&(buttons[i+1][j+1].isEnabled() == true))
		{
			leftClickAction(i+1,j+1);
		}
		if((i!=0)&&(j!=0)&&(buttons[i-1][j-1].getText()=="0")&&(buttons[i-1][j-1].isEnabled() == true))
		{
			leftClickAction(i-1,j-1);
		}
		if((i!=buttons.length-1)&&(j!=0)&&(buttons[i+1][j-1].getText()=="0")&&(buttons[i+1][j-1].isEnabled() == true))
		{
			leftClickAction(i+1,j-1);
		}
		if((i!=0)&&(j!=buttons[0].length-1)&&(buttons[i-1][j+1].getText()=="0")&&(buttons[i+1][j+1].isEnabled() == true))
		{
			leftClickAction(i-1,j+1);
		}
	}
